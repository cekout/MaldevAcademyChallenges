...
/bin/echo -e $(for i in $(objdump -d $EXES_DIR/shellcode.exe | grep "^ " | cut -f2); do echo -n '\x'$i; done;) > $BINS_DIR/shellcodeTemplate.bin && echo "Shellcode template: $BINS_DIR/shellcodeTemplate.bin";
